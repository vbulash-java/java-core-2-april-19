...
public class Robot implements Creature {
	...
	public boolean run(Tool tool) {
		...
		if(tool.getLength() > this.distance) {
			System.out.printf("Робот не смог пробежать %d - предел %d\n", tool.getLength(), this.distance);
			return false;
		} else {
	...
	public boolean jump(Tool tool) {
		...
		if(tool.getLength() > this.height) {
			System.out.printf("Робот не смог прыгнуть вверх на %d - предел %d\n", tool.getLength(), this.height);
			return false;
		} else {
			System.out.printf("Робот прыгнул вверх на %d\n", tool.getLength());
			return true;
		}
	}
}
